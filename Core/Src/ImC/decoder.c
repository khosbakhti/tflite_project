...
int getImage(JPEGDRAW *pdraw){
	//printf("Being called for drawing\r\n");
	int iCount;
	iCount = pdraw->iWidth * pdraw->iHeight;
	//dst_image[iCount] = pdraw->pPixels;
//	printf("The height and the width are %d and %d \r\n", pdraw->iHeight, pdraw->iWidth);
//	printf("The x and the y are %d and %d \r\n", pdraw->x, pdraw->y);

	//printf("Depth of the pixel %d \r\n", pdraw->iBpp);
	for(int i = 0, ii=pdraw->y; i < pdraw->iHeight; i++,ii++){
		for(int j = 0; j < pdraw->iWidth; j++){
			//dst_image[j][ii] = pdraw->pPixels[i * pdraw->iWidth + j];
			uint32_t rgb = (uint32_t)pdraw->pPixels[i * pdraw->iWidth + j];
			uint32_t R5 = ((((rgb >> 11) & 0x1F) * 527) + 23) >> 6;
			uint32_t G6 = ((((rgb >> 5) & 0x3F) * 259) + 33) >> 6;
			uint32_t B5 = (((rgb & 0x1F) * 527) + 23) >> 6;
			uint32_t a = 0;


			(*dst_image)[ii][j][0] = (int8_t) ( (R5 / 255.0)  / scale + zero_point);
			(*dst_image)[ii][j][1] = (int8_t) ( (G6 / 255.0)  / scale + zero_point);
			(*dst_image)[ii][j][2] = (int8_t) ( (B5 / 255.0)  / scale + zero_point);


			//dst_image[ii][j] = (uint32_t) R5  | G6 << 8 | B5 << 16;
...
int decodeMyImage(uint8_t* _pdata, int iDataSize, int x, int y, int iOptions, JPEG_DRAW_CALLBACK *pfnDraw, int convert_to_grayscale){
		...
		_jpeg.pfnDraw = getImage;
		_jpeg.pfnOpen = NULL;
		_jpeg.pfnClose = NULL;
		_jpeg.JPEGFile.iSize = iDataSize;
		_jpeg.JPEGFile.pData = _pdata;
		...
		//printf("Image size: %d x %d, orientation: %d, bpp: %d\r\n", _jpeg.iWidth,jpeg.iHeight, (int)_jpeg.ucOrientation, (int)_jpeg.ucBpp);
